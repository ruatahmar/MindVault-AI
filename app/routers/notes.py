...
from app.utils.gemini import get_summary
router = APIRouter(
    prefix="/notes",
    tags=["Notes"]
)
...
def generate_note(id:int, current_user:dict=Depends(get_current_user), db:Session = Depends(get_db)):
    ...
    summary = get_summary(current_post.content)
    current_post.summary = summary
    db.commit()
